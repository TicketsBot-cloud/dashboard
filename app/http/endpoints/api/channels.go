...
func ChannelsHandler(ctx *gin.Context) {
	...
	if ctx.Query("refresh") == "true" {
		...
		if err != nil {
			_ = ctx.AbortWithError(500, app.NewError(err, "Failed to take channel refresh token for guild %d. Please try again."))
			return
		}
		...
		if hasToken {
			...
			if err != nil {
				_ = ctx.AbortWithError(500, app.NewError(err, "Unable to load channels from Discord. Please try again."))
				return
			}

			if err := cache.Instance.StoreChannels(ctx, channels); err != nil {
				_ = ctx.AbortWithError(500, app.NewError(err, "Failed to store channels in cache for guild %d. Please try again."))
				return
			}
		} else {
			channels, err = cache.Instance.GetGuildChannels(ctx, guildId)
			if err != nil {
				_ = ctx.AbortWithError(500, app.NewError(err, "Unable to load channels. Please try again."))
				return
			}
		}
	} else {
