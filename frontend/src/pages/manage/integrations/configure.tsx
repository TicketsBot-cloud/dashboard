...
const ConfigureIntegrationPage: FC = () => {
  ...
  // from after one submit (Save Integration leaves the user here; only Remove
  // navigates away), so a forced lock from a 503 must release once the poll
  // confirms the flag is back on, otherwise the page stays locked forever after
  // a single incident even though the flag was re-enabled.
  useEffect(() => {
    if (polledLock === false) {
      setForcedLock(false);
    }
  ...
  const handleSave = async () => {
    try {
      await apiClient.integrations.updateGuildSecrets(
        guildId,
        integrationId,
        secretValues,
        SKIP_ERROR_TOAST,
      ...
      handleApiError(error, "Failed to save integration settings. Please try again.");
  ...
  return (
    <MainLayout
      title={`Edit ${integration.name} Settings`}
      subtitle="Update your secrets for this integration"
    >
      <FeatureLockBanner
        id="integration-lock-banner"
        locked={isLocked}
        ...
        <div className="w-full max-w-2xl bg-gray-800 rounded-xl overflow-hidden">
          ...
          <div className="p-4 flex flex-col gap-4">
            <p className="text-gray-500 text-sm">
              You cannot view previously submitted secrets for security reasons. The secret fields
              will show as empty, even if you have previously submitted them.
            </p>
