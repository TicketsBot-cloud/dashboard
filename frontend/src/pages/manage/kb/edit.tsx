...
const EditKBArticlePage: FC = () => {
  const { guildId: guildIdParam, articleId: articleIdParam } = useParams();
  const guildId = guildIdParam!;
  const articleId = Number(articleIdParam);

  const navigate = useNavigate();
  const { data: article, isLoading: articleLoading } = useKBArticle(guildId, articleId);
  const { data: categories } = useKBCategories(guildId);
  const updateArticle = useUpdateKBArticle(guildId);
  ...
  useEffect(() => {
    if (article && !initialised) {
      ...
      setSelectedCategories(article.category_ids.map(String));
      setKeywords(article.keywords || []);
      setPublished(article.published);
      setShowHelpfulCount(article.show_helpful_count);

      if (article.embed) {
        setUseEmbed(true);
        const embedColour = (() => {
          const c = article.embed.colour;
        ...
        setEmbed({
          ...
          ...article.embed,
          colour: embedColour,
          author: { ...defaultEmbed.author, ...article.embed.author },
          footer: { ...defaultEmbed.footer, ...article.embed.footer },
          fields: article.embed.fields?.length ? [...article.embed.fields] : [],
        });
      } else {
        setUseEmbed(false);
  ...
  if (articleLoading) {
    return (
      <MainLayout title="Edit Article" subtitle="Loading article...">
  ...
  if (!article && !articleLoading) {
    return (
      <MainLayout title="Article Not Found" subtitle="The requested article could not be found.">
        <div className="text-center">
          <Button
            variant="ghost"
            type="button"
            onClick={() => navigate(`/manage/${guildId}/kb`)}
            className="items-center justify-center text-blue-400 hover:text-blue-300 text-sm p-0"
          >
  ...
  return (
    <MainLayout title="Edit Article" subtitle={`Editing "${article?.title || ""}"`}>
